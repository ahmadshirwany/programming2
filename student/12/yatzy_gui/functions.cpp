...
namespace {
  ...
  bool is_straight(const map < int, int > & statistics) {
    if (statistics.size() == 5 and(statistics.find(1) == statistics.end() or statistics.find(6) == statistics.end())) {
      return true;
    } else {
      return false;
    }
...
int roll_dice() {
  ...
  static uniform_int_distribution < int > generator {
    1,
    6
  };
